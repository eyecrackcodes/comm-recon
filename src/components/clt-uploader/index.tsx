...
interface CLTUploaderProps {
  onDataLoaded: (data: any[][], headers: string[], office: "charlotte") => void;
}
...
export function CLTUploader({ onDataLoaded }: CLTUploaderProps) {
  ...
  const validateData = (headers: string[], data: any[][]): ValidationResult => {
    ...
    // Charlotte-specific validations
    const sampleSize = Math.min(1000, data.length);
    let charlotteAgentCount = 0;
    let emptyValues = 0;
    let invalidDates = 0;
    ...
    for (let i = 0; i < sampleSize; i++) {
      ...
      // Check for Charlotte office codes in agent names
      ...
      if (agentIndex >= 0 && row[agentIndex]) {
        ...
        if (
          agent.includes(" CCC") ||
          agent.toLowerCase().includes("charlotte")
        ) {
          charlotteAgentCount++;
        }
      }
    ...
    // Warn if file doesn't appear to contain Charlotte data
    const charlottePercentage = (charlotteAgentCount / sampleSize) * 100;
    if (charlottePercentage < 50) {
      warnings.push(
        `Only ${charlottePercentage.toFixed(
          1
        )}% of sampled records appear to be Charlotte agents. Please verify this is the correct CLT file.`
      );
    }
    ...
    (acceptedFiles: File[]) => {
      ...
      const processData = (headers: string[], data: any[][]) => {
        ...
        if (validationResult.isValid) {
          onDataLoaded(data, headers, "charlotte");
        } else {
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-semibold text-gray-900">
          Charlotte Call Center (CLT) Commission Data
        </h3>
      </div>
      ...
      >
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2 mb-4">
            <Upload
              className={`w-10 h-10 ${
                isUploading ? "text-gray-400" : "text-purple-500"
              }`}
            />
            <Building2
              className={`w-6 h-6 ${
          ...
          {isUploading ? (
            <p className="text-lg text-gray-600">Processing CLT file...</p>
          ) : isDragActive ? (
            <p className="text-lg text-purple-600">Drop the CLT file here...</p>
          ) : (
            <div>
              <p className="text-gray-600 font-medium">
                Upload Charlotte Commission Data
              </p>
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-md text-left max-w-md w-full">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-sm">
                  CLT Data Quality Score: {validation.dataQualityScore}/100
                </span>
              </div>
